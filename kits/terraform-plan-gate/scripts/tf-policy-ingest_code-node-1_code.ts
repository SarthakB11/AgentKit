// Default policy set. Send your own as `policies: [string]` (JSON-encoded
// {policy_id, title, text, minimum_risk?} objects) on the trigger to replace it.
// minimum_risk (low|medium|high|critical) is a floor: a change that cites the
// policy is rated at least that level, whatever the model said.
const defaults = [
  {
    "policy_id": "POL-01",
...
const supplied = {{triggerNode_1.output.policies}};
const parsed = (Array.isArray(supplied) ? supplied : [])
  .map(p => { if (typeof p === "string") { try { return JSON.parse(p); } catch (e) { return null; } } return p; })
  .filter(p => p && typeof p.policy_id === "string" && typeof p.title === "string" && typeof p.text === "string");
const RISK_LEVELS = ["low", "medium", "high", "critical"];
const floorOf = p => (typeof p.minimum_risk === "string" && RISK_LEVELS.includes(p.minimum_risk)) ? p.minimum_risk : "low";
const policies = parsed.length > 0 ? parsed : defaults;

output = {
  source: parsed.length > 0 ? "request" : "defaults",
  texts: policies.map(p => p.policy_id + " " + p.title + ". " + p.text),
  metadata: policies.map(p => ({ policy_id: p.policy_id, title: p.title, text: p.text, minimum_risk: floorOf(p) }))
};
